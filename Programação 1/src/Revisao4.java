...
public class Revisao4 {
    static void primeirosInscritos(String listaM[], String listaH[]) {
        ArrayList<String> primeirosInscritos = new ArrayList<String>();
        for (int i = 0; i < 7; i++) {
            primeirosInscritos.add(listaM[i]);
            primeirosInscritos.add(listaH[i]);
        }
    ...
    static void contratados(int idadeM[], int idadeH[], String listaM[], String listaH[]) {
        ArrayList<String> contratados = new ArrayList<String>();
        for (int i = 0; i < 20; i++) {
            if (idadeM[i] >= 38) {
                contratados.add(listaM[i]);
            }
        }
        for (int i = 0; i < 30; i++) {
            if (idadeH[i] >= 38) {
                contratados.add(listaH[i]);
            }
        }
    ...
    static void maisProvaveisFeminino(int idadeM[], String listaM[]) {
        ArrayList<String> maisProvaveisFeminino = new ArrayList<String>();
        for (int i = 0; i < 20; i++) {
            if (idadeM[i] >= 35) {
                maisProvaveisFeminino.add(listaM[i]);
            }
        }
    ...
    static void maisProvaveisMasculino(int idadeH[], String listaH[]) {
        ArrayList<String> maisProvaveisMasculino = new ArrayList<String>();
        for (int i = 0; i < 30; i++) {
            if (idadeH[i] >= 35) {
                maisProvaveisMasculino.add(listaH[i]);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        // Questão 1:
        primeirosInscritos(listaMulheres, listaHomens);
        // Questão 2:
        contratados(idadeMulheres, idadeHomens, listaMulheres, listaHomens);
        // Questão 3:
        maisProvaveisFeminino(idadeMulheres, listaMulheres);
        // Questão 4:
        maisProvaveisMasculino(idadeHomens, listaHomens);
    }
}
